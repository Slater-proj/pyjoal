...
async def test_verify_token_valid_header():
    ...
    with patch('app.core.auth.settings') as mock_settings:
        mock_settings.SECRET_TOKEN = TEST_SECRET_TOKEN
        result = await verify_token(header_token=TEST_SECRET_TOKEN)
        assert result == TEST_SECRET_TOKEN
...
async def test_verify_token_valid_query():
    ...
    with patch('app.core.auth.settings') as mock_settings:
        mock_settings.SECRET_TOKEN = TEST_SECRET_TOKEN
        result = await verify_token(header_token=None, query_token=TEST_SECRET_TOKEN)
        assert result == TEST_SECRET_TOKEN
...
async def test_verify_token_header_priority():
    ...
    with patch('app.core.auth.settings') as mock_settings:
        mock_settings.SECRET_TOKEN = TEST_SECRET_TOKEN
        result = await verify_token(header_token=TEST_SECRET_TOKEN, query_token='wrong-token')
        assert result == TEST_SECRET_TOKEN