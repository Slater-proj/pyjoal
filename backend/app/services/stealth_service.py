...
class StealthService:
    ...
    def _get_time_factor(self, hour: int) -> float:
        ...
        if hour in self._time_factor_cache:
            ...
            if cache_time and (datetime.now(timezone.utc) - cache_time).total_seconds() < 3600:  # 1 hour cache
    ...
    def _get_activity_factor(self, pattern: str, session_start: datetime) -> float:
        ...
        session_age_hours = (datetime.now(timezone.utc) - session_start).total_seconds() / 3600
    ...
    def get_natural_speed_variation(self, base_speed: int, torrent_hash: str) -> int:
        ...
        if cached_result and (datetime.now(timezone.utc) - cached_result['timestamp']).total_seconds() < 30:
    ...
    def get_rotated_port(self, torrent_hash: str, rotation_interval_hours: int = 24) -> int:
        ...
        session_age = datetime.now(timezone.utc) - profile['session_start']
    ...
    def get_session_stats(self, torrent_hash: str) -> Dict:
        ...
        session_duration = (datetime.now(timezone.utc) - profile['session_start']).total_seconds()
